.. zephyr:board:: sh400_bl5340

Overview
********
The SH400 BL5340 uses the Ezurio BL5340 module which is powered by a 
dual-core Nordic Semiconductor nRF5340 ARM Cortex-M33F CPU.
The nRF5340 inside the BL5340 module is a dual-core SoC based on
the Arm® Cortex®-M33 architecture, with:
...
The ``sh400_bl5340/nrf5340/cpuapp`` build target provides support for the application
core on the BL5340 module. The ``sh400_bl5340/nrf5340/cpunet`` build target provides
support for the network core on the BL5340 module. If ARM TrustZone is
used then the ``sh400_bl5340/nrf5340/cpuapp`` build target provides support for the
...
The BL5340 has two external oscillators. The frequency of
...
The BL5340 incorporates two sensors for user application testing.
...
Refer to the `Analog Devices MAX31341 datasheet`_ for further details.

Audio Codec
===

An Ultra-Low Power Stereo Audio Codec.

Refer to the `Analog Devices MAX98090 datasheet`_ for further details.
...
Applications built for the ``sh400_bl5340/nrf5340/cpuapp`` board by default
...
1. Build the Non-Secure Zephyr application
   for the application core using ``-DBOARD=sh400_bl5340/nrf5340/cpuapp/ns``.
...
2. Build the application firmware for the network core using
   ``-DBOARD=sh400_bl5340/nrf5340/cpunet``.
...
1. Build the Secure Zephyr application for the application core
   using ``-DBOARD=sh400_bl5340/nrf5340/cpuapp`` and
...
2. Build the Non-Secure Zephyr application for the application core
   using ``-DBOARD=sh400_bl5340/nrf5340/cpuapp/ns``.
...
and :ref:`application_run`), using ``-DBOARD=sh400_bl5340/nrf5340/cpuapp`` for
the firmware running on the BL5340's application core, and using
``-DBOARD=sh400_bl5340/nrf5340/cpunet`` for the firmware running
...
.. note::
   ...
   Flashing and debugging applications on the BL5340 requires
...
Replace :code:`<tty_device>` with the port where the BL5340 board
...
.. zephyr-app-commands::
   ...
   :board: sh400_bl5340/nrf5340/cpuapp
   :goals: build flash
...
Testing Bluetooth on the BL5340
***********************************
Many of the Bluetooth examples will work on the BL5340.
Try them out:
