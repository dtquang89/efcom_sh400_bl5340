                                       ...
                                       uint8_t addtitonal_address_offset)
{
    ...
    for (i = 0U; i < write_cycles; i++) {
        ...
        if (flash_write(flash_dev, offset, &test_data[i * write_block_size], write_block_size) != 0) {
            printf("	Write failed!\n");
            return;
        }
        printf("	Reading %u data bytes from 0x%x\n", write_block_size, offset);
        if (flash_read(flash_dev, offset, &read_buffer[i * write_block_size], write_block_size) != 0) {
            printf("	Read failed!\n");
            return;
        }
    }
...
int main(void)
{
    ...
    erase_when_needed(flash_dev, flash_params_get_erase_cap(&flash_params) & FLASH_ERASE_C_EXPLICIT, TEST_PARTITION_OFFSET,
                      FLASH_PAGE_SIZE);
    ...
    erase_when_needed(flash_dev, flash_params_get_erase_cap(&flash_params) & FLASH_ERASE_C_EXPLICIT, offset, FLASH_PAGE_SIZE * 2);
    ...
    if (!rc) {
        printf("   Page of number %u has start offset 0x%08lx\n", FLASH_TEST_PAGE_IDX, (unsigned long)info.start_offset);
